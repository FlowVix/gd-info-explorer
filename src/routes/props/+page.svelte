...
<div class="w-full h-full flex justify-center items-center gap-4">
    <div class="min-w-[70%] max-w-[70%] h-full flex flex-col gap-2">
        ...
        >
            {#each [...Object.entries(SCHEMA.classes)] as [name, cls], idx}
                {@const nameIncludes = name.toLowerCase().includes(propSearchT)}
                {@const matchingFields = nameIncludes
                    ? cls.fields
                    : cls.fields.filter(
                          field =>
                              field.name.toLowerCase().includes(propSearchT) ||
                              field.type.toLowerCase().includes(propSearchT) ||
    ...
    <div class="min-w-[30%] max-w-[30%] h-full flex flex-col gap-2">
            ...
            {#each [...Object.entries(SCHEMA.types)] as [name, type]}
                ...
                >
                    <div class="flex justify-between">
                        <span class="text-xl font-bold">{name}</span>
                        <span class="text-[#9696E1] font-bold">{type.type}</span
                        >
                    </div>
                    {#if type.type == "builtin"}
                        <span class="text-[#9696E1]">{type.description}</span>
